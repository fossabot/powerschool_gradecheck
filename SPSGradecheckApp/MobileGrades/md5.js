...
export default class md5 {
    ...
    */
    ...
    if (!unHidden) {
        ...
        pw2 = pw;
        pw = this.b64_md5(pw); // Added in move to pcas
        form.password = this.hex_hmac_md5(pskey, pw);
    ...
    if (i < 0) {
        ...
        translatorpw = this.b64_md5(translatorpw); // Added in move to pcas
        form.translator_password = this.hex_hmac_md5(pskey, translatorpw);
    ...
    var b64pw = this.b64_md5(originalpw);
    var hmac_md5pw = this.hex_hmac_md5(pskey, b64pw)
    form.pw = hmac_md5pw;
    form.dbpw = this.hex_hmac_md5(pskey, originalpw.toLowerCase())
    if (form.ldappassword != null) {
        // LDAP is enabled, so send the clear-text password
        // Customers should have SSL enabled if they are using LDAP
    ...
    return true;
     ...
     encryptMultipleStudentsAccesPasswords(form) {
    ...
    while (pw != undefined) {
        ...
        if (pw != undefined && trim(pw) != "") {
            ...
            var pw2 = this.b64_md5(pw);
            pw = this.hex_hmac_md5(pskey, pw2);
    ...
    return true;
     ...
     encryptSingleStudentAccesPassword(form) {
    ...
    if (pw != undefined && trim(pw) != "") {
        var unEncryptedPassword = pw;
        var pw2 = this.b64_md5(pw);
        pw = this.hex_hmac_md5(form.pskey, pw2);
        form.pskey = "";
    ...
    //This  has been refactored to use parameters for the element Id's so that it can be reused.
    return encryptGuardianPasswords(form, "myForm_accountInfo_password", "editedPassword", "passwordConfirm");
     ...
     encryptGuardianPasswords(form, origPasswordId, newPasswordId, newConfirmPasswordId) {
    ...
    return true;
     ...
     encryptRecoverPasswords(form, newPasswordId, newConfirmPasswordId) {
    ...
    return stringToTrim.replace(/^\s+|\s+$/g, "");
     ...
     encrypt(form, cb) {
        if (form.ldappassword != undefined && trim(form.ldappassword) != "") {
            ...
            var pw2 = this.b64_md5(form.ldappassword);
            form.pw = this.hex_hmac_md5(form.contextData, pw2);
            cb();
            //alert("unEncrypted PW: " + unEncryptedPassword+ " Encrypted Password: " + pw);
        }
    }
